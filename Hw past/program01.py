...
def decode_XKCD_tuple(xkcd_values : tuple[str, ...], k : int) -> list[int]:
    ...
    for xkcd in xkcd_values:
        listaval.append(decode_value(xkcd))
    listaval.sort(reverse=True)
    listaval=listaval[:k]'''
    listaval=list(map(decode_value,xkcd_values))
    listaval.sort(reverse=True)
    listaval=listaval[:k]
    return listaval
    pass
...
def xkcd_to_list_of_weights(xkcd : str) -> list[int]:
    ...
    for i in range(n):
     valore+=xkcd[i]
     if ((xkcd[i]=="0" and xkcd[i+1]!="0") or (xkcd[i]!="0" and xkcd[i+1]!="0")):
            valore+=" "
    valore+=xkcd[-1]
    valore=list(map(int, valore.split()))
    return valore
    pass
