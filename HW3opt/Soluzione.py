...
def ex1(ftesto,a,b,n):
    with open( ftesto, mode='r', encoding='utf8') as file :
     sequenza = file.read().strip()
     sequenza=sequenza.replace('\n','')
     frequenze = {}
     for i in range(len(sequenza)-1):
         for j in range( i+a , i+b+1):
             if j >len(sequenza) : break
             sottosequenza = sequenza[i:j]
             if sottosequenza not in frequenze:
                frequenze[sottosequenza] = 1
             else:  frequenze[sottosequenza]+=1
     dictz={}
     for chiave , valore in frequenze.items():
             if valore not in dictz:
                  dictz[valore]=[chiave]
             else:
                 dictz[valore].append(chiave)
                 
     lista = [tuple((chiave,valore)) for chiave,valore in dictz.items() ]
     lista = sorted(lista, key=lambda x: (x[0]),reverse=True)
     lista=lista[:n].copy()
     lista = sorted(lista, key=lambda x: (x[0]))
     lista = [(x[0], sorted(x[1])) for x in lista]
     return lista
     pass
